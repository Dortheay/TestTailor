import unittest
import timeout_decorator
import ansible.module_utils.common.parameters as module_0

class Test_Parameters_12(unittest.TestCase):
    @timeout_decorator.timeout(5)
    def test_case_11(self):
        int_0 = 1328
        list_0 = [int_0, int_0, int_0]
        float_0 = 4504.210798056921
        set_0 = {int_0, float_0, int_0}
        tuple_0 = (list_0, float_0, float_0, set_0)
        list_1 = [set_0, set_0]
        var_0 = module_0.remove_values(tuple_0, list_1)
        str_0 = 'tFB"jU00p>}RXD62'
        var_1 = module_0.remove_values(list_1, str_0)
        str_1 = '&t<X_V:dR=VP(62No'
        var_2 = module_0.sanitize_keys(tuple_0, str_1)

if __name__ == "__main__":
    unittest.main()
