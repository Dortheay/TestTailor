import unittest
import timeout_decorator
import typed_ast._ast3 as module_0
import py_backwards.transformers.dict_unpacking as module_1

class Test_Dict_unpacking_6(unittest.TestCase):
    @timeout_decorator.timeout(5)
    def test_case_2(self):
        tuple_0 = ()
        list_0 = [tuple_0, tuple_0, tuple_0, tuple_0]
        list_1 = [list_0, list_0]
        dict_0 = module_0.Dict(*list_1)
        dict_1 = {}
        a_s_t_0 = module_0.AST(**dict_1)
        a_s_t_1 = module_0.AST()
        dict_unpacking_transformer_0 = module_1.DictUnpackingTransformer(a_s_t_1)
        var_0 = dict_unpacking_transformer_0.visit_Dict(dict_0)
        a_s_t_2 = module_0.AST(**dict_1)
        dict_unpacking_transformer_1 = module_1.DictUnpackingTransformer(a_s_t_2)
        module_x_var_0 = module_0.Module(*list_1)
        dict_unpacking_transformer_2 = module_1.DictUnpackingTransformer(a_s_t_1)
        module_x_var_1 = dict_unpacking_transformer_2.visit_Module(module_x_var_0)

if __name__ == "__main__":
    unittest.main()
